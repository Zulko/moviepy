...
Note that some of these functions such as ``audio_volume`` (which multiplies the volume) can
...
You can either import a single function like this: ::
    ...
    from moviepy.audio.fx.audio_volume import audio_volume
    newaudio = audioclip.fx(audio_volume, 0.5)

Or import everything: ::
    
    import moviepy.audio.fx.all as afx
    newaudio = (audioclip.afx( vfx.normalize)
                         .afx( vfx.audio_volume, 0.5)
...
the module ``audio.fx`` is loaded as ``afx`` and you can use ``afx.audio_volume``, etc.
