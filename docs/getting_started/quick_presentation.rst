...
This section explains when MoviePy can be used, how it can be used, and how it works.

Do I need MoviePy ?
~~~~~~~~~~~~~~~~~~~

There are already many good free video editing softwares out there with nice graphical interfaces (my favorites being OpenShot and KDEnlive). So why write videos using scripts ? Here are a few reasons:
...
- You want to create animations from images generated by another python library (Matplotlib, Mayavi, Gizeh...)
...
Now for the limitations: MoviePy cannot stream videos (read from a webcam, or render a video live on a distant machine), it cannot do 3D effects (by lack of a nice 3D library for Python) and is not well designed for video processing involving successive frames of a movie (like video stabilization, you'll need another software for that). Also, if you only need to do frame-by-frame video analysis (with face detection or other fancy stuff) you may want to use the libraries OpenCV and SimpleCV instead.


Example code
~~~~~~~~~~~~~~

In a typical MoviePy script, you load some video or audio files, modify them, put them together, and write the result to a new video file. As an example, let us load a video of my last holidays, lower the volume, add a title in the center of the video for the first ten seconds, and write the result in a file: ::
...
MoviePy uses the software ``ffmpeg`` to read and to export video and audio files. It also (optionally) uses ImageMagick to generate texts and write GIF files. The processing of the different media is ensured by Python's fast numerical library Numpy, and advanced effects and enhancements can be done using some of Python's numerous image processing library (PIL, Scikit-image, scipy, etc.).
...
You will also find a few advanced goodies in ``moviepy.video.tools`` to track objects in a video, draw simple shapes and color gradients (very useful for masks), generate subtitles and end credits, etc. See :ref:`advanced_tools` for a description of these.
