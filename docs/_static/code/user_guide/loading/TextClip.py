...
txt_clip1 = TextClip(font=font, text="Hello World !", font_size=30, color="#FF0000", bg_color="#FFFFFF", duration=3) # Red
...
txt_clip2 = TextClip(font=font, filename="./example.txt", size=(500, 200), bg_color="#FFFFFF", method="caption", color=(0, 0, 255, 127)) # Blue with 50% transparency

# we set duration, because by default image clip are infinite, and we cannot render infinite
txt_clip2 = txt_clip2.with_duration(3) 

txt_clip1.write_videofile("result1.mp4", fps=24) # ImageClip have no FPS either, so we must defined it
