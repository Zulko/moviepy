...
def autoTrack(clip, pattern, tt=None, fps=None, radius=20, xy0=None):
    ...
    Returns ``[(x1, y1), (x2, y2)...]`` where xi,yi are the coordinates of the
    pattern in the clip on frame ``i``. To select the frames you can either
    specify a list of times with ``tt`` or select a frame rate with ``fps``.
